...
struct PocketDimension {
    // 4D array of 20 by 20 by 13 by 13
    //
    ...
    cubes: [[[[Cube; 13]; 13]; 20]; 20],
}

impl PocketDimension {
    fn new() -> PocketDimension {
        PocketDimension {
            cubes: [[[[Cube { active: false }; 13]; 13]; 20]; 20],
        }
    }
    ...
    fn from(input: &str) -> PocketDimension {
        ...
        for (i, line) in input_lines.iter().enumerate().take(8) {
            for (j, char) in line.chars().enumerate() {
                result.cubes[6 + i][6 + j][6][6].active = match char {
                    '#' => true,
                    '.' => false,
    ...
    fn count_active_neighbours(&self, x: usize, y: usize, z: usize, w: usize) -> u8 {
        if x >= 20 || y >= 20 || z >= 13 || w >= 13 {
        ...
        for i in max(0, x as isize - 1) as usize..=min(19, x + 1) {
            for j in max(0, y as isize - 1) as usize..=min(19, y + 1) {
                for k in max(0, z as isize - 1) as usize..=min(12, z + 1) {
                    for l in max(0, w as isize - 1) as usize..=min(12, w + 1) {
                        if i == x && j == y && k == z && l == w {
                            continue;
                        } else if self.cubes[i][j][k][l].active {
                            active_neighbours += 1;
                        }
                    }
                }
        ...
        for x in 0..self.cubes.len() {
            for y in 0..self.cubes[0].len() {
                for z in 0..self.cubes[0][0].len() {
                    for w in 0..self.cubes[0][0][0].len() {
                        next.cubes[x][y][z][w].active = matches!(
                            (
                                self.cubes[x][y][z][w].active,
                                self.count_active_neighbours(x, y, z, w),
                            ),
                            (true, 2) | (_, 3)
                        )
                    }
    ...
    fn display(&self) {
        for w in 0..self.cubes[0][0][0].len() {
            for z in 0..self.cubes[0][0].len() {
                println!("\nz = {}, w = {}", z as i32 - 6, w as i32 - 6);
                for x in 0..self.cubes[0].len() {
                    println!();
                    for y in 0..self.cubes.len() {
                        let symbol = match self.cubes[x][y][z][w].active {
                            true => '#',
                            false => '.',
                        };
                        print!("{}", symbol);
